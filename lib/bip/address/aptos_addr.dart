  ...
  static final multiKeyAddressLayout = LayoutConst.struct([
    LayoutConst.bcsVector(
        LayoutConst.bcsLazyEnum([
          LazyVariantModel(
              layout: LayoutConst.bcsBytes, property: EllipticCurveTypes.ed25519.name, index: 0),
          LazyVariantModel(
              layout: LayoutConst.bcsBytes, property: EllipticCurveTypes.secp256k1.name, index: 1)
        ], property: "pubKey"),
        property: 'publicKeys'),
...
class AptosAddressUtils {
  ...
  static List<int> praseAddressBytes(List<int> bytes) {
    ...
    if (length != AptosAddrConst.addressBytesLength) {
      throw AddressConverterException("Invalid aptos address bytes length.",
          details: {"expected": AptosAddrConst.addressBytesLength, "length": bytes.length});
    }
  ...
  static List<int> addressToBytes(String address) {
    ...
    List<int>? bytes = BytesUtils.tryFromHexString(address,
        paddingZero: address.length == 1 || address.length == AptosAddrConst.shortAddressLength);
    if (bytes == null || (bytes.length != AptosAddrConst.addressBytesLength && bytes.length != 1)) {
      throw AddressConverterException("Invalid aptos address.", details: {"address": address});
    }
    if (bytes.length == 1) {
      final byte = bytes[0];
  ...
  static List<int> hashKeyBytes({required List<int> bytes, required int scheme}) {
    bytes = [...bytes, scheme];
  ...
  static List<int> encodeEd25519Key(List<int> bytes) {
    try {
      final key = AddrKeyValidator.validateAndGetEd25519Key(bytes).compressed.sublist(1);
      return hashKeyBytes(bytes: key, scheme: AptosAddrConst.ed25519AddressFlag);
    } catch (e) {
  ...
  static List<int> encodeSingleKey(IPublicKey publicKey) {
    try {
      ...
      final encode = AptosAddrConst.singleKeyAddressLayout.serialize(structLayout);
      return hashKeyBytes(bytes: encode, scheme: AptosAddrConst.signleKeyAddressFlag);
  ...
  static List<int> encodeMultiEd25519Key(List<Ed25519PublicKey> publicKeys, int threshold) {
    try {
      ...
      if (threshold < AptosAddrConst.minthreshold || threshold > publicKeys.length) {
        throw AddressConverterException(
      ...
      return hashKeyBytes(bytes: keyBytes, scheme: AptosAddrConst.multiEd25519AddressFlag);
    } on AddressConverterException {
      rethrow;
    } catch (e) {
      throw AddressConverterException("Invalid aptos MultiEd25519 address bytes.",
          details: {"error": e.toString()});
    }
  }

  /// encode Multi Public keys to MultiKey address
  static List<int> encodeMultiKey(List<IPublicKey> publicKeys, int requiredSignature) {
    try {
      ...
      final encode = AptosAddrConst.multiKeyAddressLayout.serialize(layoutStruct);
      return hashKeyBytes(bytes: encode, scheme: AptosAddrConst.multikeyAddressFlag);
    } on AddressConverterException {
      rethrow;
    } catch (e) {
      throw AddressConverterException("Invalid aptos MultiKey address bytes.",
...
class AptosSingleKeyEd25519AddrEncoder implements BlockchainAddressEncoder {
  ...
  String encodeKey(List<int> pubKey, [Map<String, dynamic> kwargs = const {}]) {
    ...
    return BytesUtils.toHexString(addressBytes, prefix: CoinsConf.aptos.params.addrPrefix);
  }
}
...
class AptosAddrEncoder implements BlockchainAddressEncoder {
  ...
  String encodeMultiEd25519Key(
    ...
    final addressBytes = AptosAddressUtils.encodeMultiEd25519Key(publicKeys, threshold);
    return BytesUtils.toHexString(addressBytes, prefix: CoinsConf.aptos.params.addrPrefix);
  }

  /// This method is used to create an Aptos `MultiKey` address from (ED25519 or Secp256k1) public keys.
  String encodeMultiKey({required List<IPublicKey> publicKeys, required int requiredSignature}) {
    final addressBytes = AptosAddressUtils.encodeMultiKey(publicKeys, requiredSignature);
    return BytesUtils.toHexString(addressBytes, prefix: CoinsConf.aptos.params.addrPrefix);
  }
}
