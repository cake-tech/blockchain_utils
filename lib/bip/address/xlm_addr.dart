...
class XlmAddrTypes {
  ...
  static const XlmAddrTypes contract = XlmAddrTypes._(value: 2 << 3, name: "Contract");

  /// Public key address type. (keyBytes must be a valid Ed25519 public key)
  static const XlmAddrTypes pubKey = XlmAddrTypes._(name: "PublicKey", value: 6 << 3);

  /// Private key address type. (keyBytes must be a valid Ed25519 private key)
  static const XlmAddrTypes privKey = XlmAddrTypes._(name: "SecretKey", value: 18 << 3);

  /// Muxed
  static const XlmAddrTypes muxed = XlmAddrTypes._(value: 12 << 3, name: "Muxed");
  ...
  static XlmAddrTypes fromTag(int? tag) {
    return values.firstWhere((e) => e.value == tag,
        orElse: () => throw AddressConverterException("Invalid or unsuported xlm address type.",
            details: {"expected": values.map((e) => e.value).join(", "), "got": tag}));
  }
...
class XlmAddrDecoder implements BlockchainAddressDecoder {
  XlmAddrDecoderResult decode(String addr, [Map<String, dynamic> kwargs = const {}]) {
    final addrType = AddrKeyValidator.nullOrValidateAddressArgs<XlmAddrTypes>(kwargs, "addr_type");
    final addrDecBytes = Base32Decoder.decode(addr);

    final payloadBytes =
        AddrDecUtils.splitPartsByChecksum(addrDecBytes, XlmAddrConst.checksumByteLen).item1;
    ...
    AddrDecUtils.validateBytesLength(addrDecBytes,
        type == XlmAddrTypes.muxed ? XlmAddrConst.muxedAddrLen : XlmAddrConst.pubkeyAddrLength);
    ...
    if (type == XlmAddrTypes.muxed) {
      ...
      if (accountId > maxU64 || accountId < BigInt.zero) {
        throw const AddressConverterException("Invalid muxed address account id.");
      }
...
class XlmAddrEncoder implements BlockchainAddressEncoder {
  ...
  String encodeKey(List<int> pubKey, [Map<String, dynamic> kwargs = const {}]) {
    if (pubKey.length == Ed25519KeysConst.pubKeyByteLen + Ed25519KeysConst.pubKeyPrefix.length) {
      pubKey = pubKey.sublist(1);
    }
    final addrType =
        AddrKeyValidator.nullOrValidateAddressArgs<XlmAddrTypes>(kwargs, "addr_type") ??
            XlmAddrTypes.pubKey;
    ...
    if (addrType == XlmAddrTypes.muxed) {
      ...
      final idBytes = BigintUtils.toBytes(muxedId, length: XlmAddrConst.muxedIdLength);
    ...
    return Base32Encoder.encodeNoPaddingBytes(List<int>.from([...payloadBytes, ...checksumBytes]));
  }
}
