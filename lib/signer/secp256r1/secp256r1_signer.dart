...
class Nist256p1Signer {
  ...
  factory Nist256p1Signer.fromKeyBytes(List<int> keyBytes) {
    final signingKey = ECDSAPrivateKey.fromBytes(keyBytes, _NistSignerConst.nist256256);
  ...
  List<int> _signEcdsa(List<int> digest, {bool hashMessage = true}) {
    ...
    if (ecdsaSign.s > _NistSignerConst.orderHalf) {
      ecdsaSign = ECDSASignature(ecdsaSign.r, _NistSignerConst.curveOrder - ecdsaSign.s);
    }
    final sigBytes = ecdsaSign.toBytes(_NistSignerConst.nist256256.curve.baselen);
  ...
  Nist256p1Verifier toVerifyKey() {
    return Nist256p1Verifier.fromKeyBytes(_ecdsaSigningKey.privateKey.publicKey.toBytes());
  }
}
...
class Nist256p1Verifier {
  ...
  bool _verifyEcdsa(List<int> digest, List<int> sigBytes) {
    final signature = ECDSASignature.fromBytes(sigBytes, _NistSignerConst.nist256256);
  ...
  bool verify(List<int> message, List<int> signature, {bool hashMessage = false}) {
    if (hashMessage) {
